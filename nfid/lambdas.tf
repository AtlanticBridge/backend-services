...
resource "aws_iam_role" "coinbase_auth_init_lambda_role" {
  name = "coinbase_auth_init_lambda_role"
...
resource "aws_iam_role_policy" "coinbase_auth_init_lambda_policy" {
  name   = "coinbase_auth_init_lambda_policy"
  role   = aws_iam_role.coinbase_auth_init_lambda_role.id
  policy = <<EOF
{
    "Version": "2012-10-17",
    "Statement": [
...
resource "aws_lambda_function" "coinbase_auth_init_lambda" {
  filename         = data.archive_file.nfid_lambda_archive.output_path
  source_code_hash = data.archive_file.nfid_lambda_archive.output_base64sha256
  publish          = true
  function_name    = "coinbase_auth_init_lambda"
  role             = aws_iam_role.coinbase_auth_init_lambda_role.arn
  handler          = "nfid_sign_in.lambda_handler"
  runtime          = "python3.8"
  timeout          = 15
